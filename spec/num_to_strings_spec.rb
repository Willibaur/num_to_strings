...
describe 'num_to_strings' do
  ...
  it("returns worded numbers when passed numbers as strings") do
    ...
    99.times do |number|
      ...
      if number < 20
        expect(num_to_strings(number.to_s)).to eq qty_1s_to19s[number]
      elsif number >= 20 && second != 0
        expect(num_to_strings(number.to_s)).to eq qty_20s_to_90s[first] +" "+ qty_1s_to19s[second]
      else number >= 20 && second == 0
        expect(num_to_strings(number.to_s)).to eq qty_20s_to_90s[first]
      end
